...
type castle_style =
  ...
  | Disabled
      (** No castling (e.g. Chess960 until 960 castling lands). *)
...
let rules_chess960 =
  { critical = King; castling = Disabled; promo_kinds = default_promo }
...
let chess960 ~seed =
  make ~castling:no_castling ~rules:rules_chess960
    (Board.of_list (Setup.chess960 ~seed))

let queer_immobile pieces = List.map fst pieces

let queer_kings =
  let pieces = Setup.queer_kings in
  make ~rules:rules_double_kings ~immobile:(queer_immobile pieces)
    (Board.of_list pieces)

let queer_queens =
  let pieces = Setup.queer_queens in
  make ~rules:rules_double_queens ~immobile:(queer_immobile pieces)
    (Board.of_list pieces)
